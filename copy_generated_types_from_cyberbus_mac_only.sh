...
pushd ../cyberbus-backend/crates
rm -rf **/bindings
popd

# First re-generate the types by running cargo test on cyberbus-backend
pushd ../cyberbus-backend/scripts
./test-with-remote-db.sh
